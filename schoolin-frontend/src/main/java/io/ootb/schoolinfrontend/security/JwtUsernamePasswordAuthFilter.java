...
public class JwtUsernamePasswordAuthFilter extends OncePerRequestFilter {
    ...
    private final SchoolINRealm realm;
    private final ObjectMapper mapper;
    
    @Autowired
    private JwtUtil jwtUtil;

    public JwtUsernamePasswordAuthFilter(JwtConfig config, SchoolINRealm realm) {
        this.config = config;
        this.realm = (SchoolINRealm) realm;
    ...
    protected void doFilterInternal(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException {
        ...
        // generate jwt token
        io.ootb.schoolinfrontend.domain.entity.User principal = (io.ootb.schoolinfrontend.domain.entity.User) subject.getPrincipal();
